    ...
    while True:
        ...
        if app.is_running():
            ...
            if state[UI_CONFIG["follow_key"]] and settings["Follow Distance"] > 0 and state['follow_target']:
                ...
                if GetDistance(follow_id) > follow_distance:
                    NewMoveXY(GetX(follow_id), GetY(follow_id), True, 1, True)
                print(f"Following {state['follow_target'][1]} at {follow_distance} tiles")
            
            # Mortal Strike
            if state['MORTAL']:
                if settings["Remove Mortal (Friends)"]:
                    print("Removing Mortal Strike for friends")
                    # Implement Remove Mortal logic here
                
                if settings["Remove Mortal (Pets)"]:
                    print("Removing Mortal Strike for pets")
                    # Implement Remove Mortal logic here
            
            # Bandage
            if state["BANDAGE"] and settings["Use Bandages (Friends)"]:
                threshold = settings["Friend Bandage Threshold"]
                print(f"Bandaging friends at {threshold}% health")
                # Implement bandaging logic here
            
            # Cure
            if state["CURE"]:
                threshold = settings["Friend Cure Threshold"]
                print(f"Curing friends at {threshold}% poison")
                # Implement curing logic here
            
            # Heal
            if state["HEAL"]:
                threshold = settings["Friend Heal Threshold"]
                print(f"Healing friends at {threshold}% health")
                # Implement healing logic here
            
            # Veterinary
            if settings["Use Veterinary"]:
                print("Using Veterinary on pets")
                # Implement Veterinary logic here
        ...
        time.sleep(settings["Scan Frequency (ms)"] / 1000)
